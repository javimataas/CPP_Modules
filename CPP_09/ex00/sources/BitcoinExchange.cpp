...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/23 10:02:08 by jmatas-p          #+#    #+#             */
/*   Updated: 2024/08/07 19:37:47 by jmatas-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static float stringTof(std::string const &str)
{
...
static int stringToi(std::string const &str)
{
	std::stringstream	ss(str);
	int					ret;
...
{
	int year = stringToi(date.substr(0, 4));
	int month = stringToi(date.substr(5, 2));
	int day = stringToi(date.substr(8, 2));
...
void	BitcoinExchange::takeRates(std::string const &filename)
{
	...
	if (rateData.is_open())
	{
		while (std::getline(rateData, line))
		{
			...
			try
			{
				value = stringTof(line.substr(line.find(",") + 1));
			}
